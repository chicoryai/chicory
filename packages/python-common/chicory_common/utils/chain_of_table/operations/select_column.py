...
import re
import numpy as np
from services.utils.chain_of_table.utils.helper import table2df, NoIndent, MyEncoder

from services.utils.chain_of_table.third_party.select_column_row_prompts.select_column_row_prompts import select_column_demo
...
def select_column_act(table_info, operation, union_num=2, skip_op=[]):
    ...
    new_table = []
    for cols in table:
        if cols[0] and cols[0].lower() in selected_columns:
