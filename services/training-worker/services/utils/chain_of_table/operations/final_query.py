...
import copy
import numpy as np
from services.utils.chain_of_table.utils.helper import table2string
...
def simple_query(sample, table_info, llm, debug=False, use_demo=False, llm_options=None, insight_use=False):
    ...
    if insight_use:
        prompt += "\n"
        prompt += ("Here are the questions about the table and the task is to answer those questions to the best of your"
                   "capability and data access.\n")
        prompt += ("If the question is answerable, response with detailed result, and otherwise answer the explanation"
