...
from services.models.llm_orchestrator import LLMOrchestrator, ModelType
...
def initialize_memzo_api_workflow_agent(user, project):
    ...
    orchestrator = LLMOrchestrator(project=project, global_seed=SEED)
    llm = orchestrator.get_general_llm()
    llm_mini = orchestrator.get_mini_llm()
    reasoning_llm = orchestrator.get_reasoning_llm()
    chat_llm = orchestrator.get_chat_llm()
    embedding_model_name = orchestrator.get_model_config(ModelType.EMBEDDING).model_name
    checkpoint_memory, vector_store, mem_store = initialize_memory_cache(embedding_model_name)
...
""",
    ...
    task_expert_chain = task_expert_prompt | llm.with_structured_output(Task)
...
In case, it doesn't succeed, please include the intermediate results to the final response, for transparency."""
    ...
    async def task_expert_step(state: PlanExecute):
        ...
        return {"task_info": task_information, "attributes": attributes}
...
Provide all details required to plan investigation for task:
{task_info}
...
""")
        ...
        for retry in range(RETRIES):
            try:
                ...
                return {"plan": plan_str_list, "context": state["context"], "tools": tools, "extracted_values": ""}
            except Exception as e:
                logger.error(f"Error planning: {str(e)}", exc_info=e)
                continue

        return {"plan": ["Validate environment variables. replan."], "tools": tools, "extracted_values": ""}
...
"""
        ...
        task_formatted = f"""For exploring task: `{query}`,
...
"""
        ...
        if val_agent_response_str and val_agent_response_str != "```json\n{}\n```":
            if extracted_values:
                extracted_values += f"\n\n{val_agent_response_str}"
            else:
                extracted_values = val_agent_response_str

        this_step = (task, exec_agent_response_str)
        if this_step:
            past_steps.append(this_step)

        result = { "plan": remaining_plan, "past_steps": [(task, exec_agent_response_str)] }
    ...
    async def finalize(state):
        ...
        for retry in range(RETRIES):
            try:
                ...
                logger.error(f"Error synthesize: {str(e)}", exc_info=e)
                continue
